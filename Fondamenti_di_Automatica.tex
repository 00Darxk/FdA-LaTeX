...
%   Linearization;
%   Rumori Aleatori;
%   Figura Operazionale in Frequenza;
%   Reti Correttrici Grafico Circuito;
%   Grafico Quantizzazione;
%   Grafico Temporale Tc elaborazione controllore;
%   Grafico Impulso Discreto x(t);
%   Grafico Piano di Gauss x_c(s);
...
l'uscita in tensone del processo all'ingresso in tensione dei convertitori, poiché i convertitori hanno un certo intervallo di ingresso accettabile in Volt, il dato 
ottenuto operando su quello grezzo del sensore viene chiamato dato ingegnerizzato. \\

Il valore di riferimento del microcontrollore può essere computato o inserito da un segnale esterno. Il microcontrollore $\mu C$ comprendo il microprocessore, ed i convertitori 
$DAC$ e $A/D$, anche su un singolo chip. 

\begin{center}
    \begin{tikzpicture}
        \node[lblock](mp)at(0,0){$\mu P$};
    \end{tikzpicture}
\end{center}

\subsubsection{Conversione Dati}

Un dato digitale viene rappresentato da un certo numero $n$ di bit, per cui può rappresentare valori nell'intervallo $[0,2^n-1]$. Viene suddiviso l'intervallo di tensione 
in entrata in $2^n$ intervalli uguali e ogni valore che un bit può rappresentare corriponsderà quindi ad un intervallo di tensione. Non è possibile determinare in questo 
modo la tensione esatta, questo effetto viene chiamato quantizzazione, ed è quantificabile dal passo di quantizzazione, ovvero la grandezza di un singolo intervallo di 
tensione:
...
\begin{center}\begin{tikzpicture}[scale=2]
    \node[rectangle, draw, very thick, minimum width=2cm, minimum height=1cm](r)at(0,0){$g(y_{i-1},y_{i-2},...;u_{i},u_{i-1},..)$};

    \draw[->,ultra thick](-2.5,0)--(r.west);
    \node[below]at(-2,0){$\{u_i\}$};

    \draw[<-,ultra thick](2.5,0)--(r.east);
    \node[below]at(2,0){$\{y_i\}$};
...
esponenziale si ripeterà ogni $\omega_c$. Per cui il piano di Guass sarà diviso in delle strisce di ampiezza $\omega_c$ uguali tra loro, allora si consdera solamente 
